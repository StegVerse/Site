(function(root,factory){
  ...
  function assertNoSecretFields(value,path){
    ...
    Object.keys(value).forEach(function(key){
      ...
      if(lower!=="raw_provider_account_identifier_present"&&FORBIDDEN.some(function(p){return lower.indexOf(p)!==-1;}))throw new Error("FAIL_CLOSED: secret or raw provider identifier prohibited at "+path+"."+key);
  ...
  function buildTransferPacket(input){
    ...
    var payload={
      owner_selection_ref:String(input.owner_selection_ref),
      provider_org_ref:String(input.provider_org_ref),
      account_class:String(input.account_class),
      account_status:String(input.account_status),
      source_evidence_ref:String(input.source_evidence_ref),
      source_evidence_sha256:String(input.source_evidence_sha256)
    };
    return sha256Value(payload).then(function(payloadHash){
      return sha256Text(payload.owner_selection_ref+"\n"+payloadHash).then(function(idHash){
        var packet={
          ...
          transfer_id:"kvskap_"+idHash.slice(7,31),
        ...
        assertNoSecretFields(packet,"packet");
        return packet;
      });
    });
  }
  function buildKVRequest(packet,requestId,authorityRef){
    ...
    return {
      ...
      candidate_writeback:{candidate_type:"SKAP_ACCOUNT_METADATA_TRANSFER",payload_ref:packet.payload_sha256,requested_destination:"skap://internal/account-metadata"}
    };
  }
  function buildBoundaryEnvelope(packet,request,intrReceiptRef){
    ...
    return Promise.all([sha256Value(request),sha256Value(packet)]).then(function(h){return {
      ...
      transfer_packet_sha256:h[1],
      payload_sha256:packet.payload_sha256,
      intr_receipt_ref:intrReceiptRef||null,
  ...
  function buildTransportMaterialization(intr,packet,request,envelope,carrierBinding){
    ...
    return Promise.resolve(intr.buildIntent(PROFILE_ID,bytes,OPERATION,request.request_id)).then(function(intent){
      return intr.buildMaterializationRequest(PROFILE_ID,intent,"inline://materialization_request.account_metadata_transfer",carrierBinding||null,{
        account_metadata_transfer:packet,
        kv_interlock_request:request,
