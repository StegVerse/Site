...
def main() -> None:
    ...
    selection = discovery.get("selection_policy", {})
    if selection.get("automatic_provider_selection") is not False or selection.get("explicit_selection_required") is not True:
        die("origin-selection policy mismatch")
    if selection.get("selected_origin") is not None:
        die("discovery state unexpectedly contains a selected origin")
